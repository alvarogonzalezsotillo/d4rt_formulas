...
void main() {
  ...
  group('MDCalc formulas are loaded', () {
    ...
    test('medical units load and convert', () async {
      ...
      expect(corpus.convert(100, 'milligram per deciliter', 'milligram per liter'), closeTo(1000, 0.001));
      // 1 g/dL = 1000 mg/dL
      expect(corpus.convert(1, 'gram per deciliter', 'milligram per deciliter'), closeTo(1000, 0.001));
      // 1 year in seconds
  ...
  group('Renal', () {
    test('Cockcroft-Gault - male', () async {
      ...
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'Age': 70.0, 'Sex': 'Male', 'Weight': 70.0, 'SerumCreatinine': 1.0});
    ...
    test('Cockcroft-Gault - female', () async {
      ...
      final formula = corpus.getFormula('Creatinine Clearance (Cockcroft-Gault)')!;
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'Age': 70.0, 'Sex': 'Female', 'Weight': 70.0, 'SerumCreatinine': 1.0});
    ...
    test('MDRD - female non-African American', () async {
      ...
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'SerumCreatinine': 1.0, 'Age': 50.0, 'Sex': 'Female', 'Race': 'Non-African American'});
    ...
    test('CKD-EPI 2021 - female', () async {
      ...
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'SerumCreatinine': 0.7, 'Age': 50.0, 'Sex': 'Female'});
  ...
  group('Electrolytes and gases', () {
    test('Anion Gap', () async {
      ...
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'Sodium': 140.0, 'Chloride': 105.0, 'Bicarbonate': 24.0});
    ...
    test('Osmolar Gap', () async {
      ...
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'Sodium': 140.0, 'Glucose': 100.0, 'BUN': 14.0, 'MeasuredOsmolality': 285.0});
    ...
    test('A-a gradient', () async {
      ...
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'FiO2': 21.0, 'PaCO2': 40.0, 'PaO2': 90.0});
  ...
  group('Liver scores', () {
    test('MELD Score', () async {
      ...
      final evaluator = FormulaEvaluator();
      final result = evaluator.evaluate(formula, {'Bilirubin': 2.0, 'INR': 1.5, 'Creatinine': 1.2, 'Dialysis': 'No'});
